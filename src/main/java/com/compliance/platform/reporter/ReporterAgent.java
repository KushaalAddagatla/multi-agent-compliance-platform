...
public class ReporterAgent {
    ...
    public ComplianceReport generateReport(LocalDate reportDate) {
        ...
        long newViolations              = countViolationsByNewness(true);
        long persistentViolations       = countViolationsByNewness(false);

        // 2. Render markdown
        String markdown = buildMarkdownReport(
                reportDate, byFramework, bySeverity, top5, scores,
                totalViolations, newViolations, persistentViolations);
        ...
        if (!fromEmail.isBlank() && !toEmail.isBlank()) {
            try {
                sendEmail(reportDate, scores, totalViolations, newViolations,
                        persistentViolations, s3Key, markdown);
    ...
    private long countViolationsByNewness(boolean isNew) {
        Long count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM violations WHERE is_new = ?",
                Long.class, isNew);
    ...
    private String buildMarkdownReport(LocalDate date,
        ...
        // Violation Summary — new vs persistent split
        sb.append("## Violation Summary\n\n");
        sb.append("| | Count |\n|---|---|\n");
        sb.append("| **Total violations** | **").append(total).append("** |\n");
        sb.append("| 🆕 New this run | ").append(newCount).append(" |\n");
        sb.append("| 🔁 Persistent (ongoing) | ").append(persistentCount).append(" |\n\n");
    ...
    private void sendEmail(LocalDate date, Map<String, Double> scores,
                           long totalViolations, long newViolations, long persistentViolations,
                           String s3Key, String markdownFallback) {
        String subject = "AWS Compliance Report — " + date
                + " (" + newViolations + " new, " + persistentViolations + " persistent)";

        // HTML body — new vs persistent split is the key addition
        var html = new StringBuilder();
        html.append("<h2>AWS Compliance Report — ").append(date).append("</h2>");

        html.append("<h3>Violation Summary</h3>");
        html.append("<table border='1' cellpadding='6' style='border-collapse:collapse'>");
        html.append("<tr><th>Category</th><th>Count</th></tr>");
        html.append("<tr><td><strong>Total</strong></td><td><strong>").append(totalViolations).append("</strong></td></tr>");
        html.append("<tr><td>&#128197; New this run</td><td style='color:#ef4444'><strong>").append(newViolations).append("</strong></td></tr>");
        html.append("<tr><td>&#128260; Persistent (ongoing)</td><td>").append(persistentViolations).append("</td></tr>");
        html.append("</table>");

        html.append("<h3>Framework Scores</h3>");
        html.append("<table border='1' cellpadding='6' style='border-collapse:collapse'>");
        ...
        if (s3Key != null) {
            html.append("<p>Full report: <code>s3://")
